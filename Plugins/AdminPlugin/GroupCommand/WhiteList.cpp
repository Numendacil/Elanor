...
bool WhiteList::Execute(const Mirai::GroupMessageEvent& gm, Bot::Group& group, Bot::Client& client,
                        Utils::BotConfig& config) 
{
	...
	if (command != "#white" && command != "#白名单" && command != "#whitelist") return false;
	...
	{
		LOG_INFO(Utils::GetLogger(), "帮助文档 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
	...
	if (command == "clear")
	{
		...
		LOG_INFO(Utils::GetLogger(), "清除成功 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
		client.SendGroupMessage(group.gid, Mirai::MessageChain().Plain("白名单归零了捏"));
		return true;
	}
	...
	if (command == "clean")
	{
		...
		LOG_INFO(Utils::GetLogger(), "整理成功 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
		client.SendGroupMessage(group.gid, Mirai::MessageChain().Plain("白名单打扫好了捏"));
		return true;
	}
	...
	if (command == "list")
	{
		...
		LOG_INFO(Utils::GetLogger(), "输出名单 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
	...
	if (command == "exist" || command == "add" || command == "del" || command == "delete")
	{
		...
		{
			LOG_INFO(Utils::GetLogger(), "缺少参数[QQ] <WhiteList>: " + command + Utils::GetDescription(gm.GetSender(), false));
		...
		for (int i = 2; i < tokens.size(); i++)
		{
			...
			{
				LOG_INFO(Utils::GetLogger(), "无效参数[QQ] <WhiteList>: " + tokens[i] + Utils::GetDescription(gm.GetSender(), false));
		...
		if (command == "exist")
		{
			...
			LOG_INFO(Utils::GetLogger(), "查询成功 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
			client.SendGroupMessage(group.gid, Mirai::MessageChain().Plain(msg));
			return true;
		}
		...
		if (command == "delete" || command == "del")
		{
			for (const auto& id : arr)
				access_list->WhiteListDelete(id);
			LOG_INFO(Utils::GetLogger(), "删除成功 <WhiteList>" + Utils::GetDescription(gm.GetSender(), false));
			client.SendGroupMessage(group.gid, Mirai::MessageChain().Plain("白名单更新好了捏"));
			return true;
		}
	}

	LOG_INFO(Utils::GetLogger(), "未知命令 <WhiteList>: " + command + Utils::GetDescription(gm.GetSender(), false));
