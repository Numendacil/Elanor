...
inline unsigned char* CensorImage(const std::string& image, double sigma, size_t& len, const std::string& cover = {})
{
	...
	if (!cover.empty())
	{
		...
		in = in.composite2(top, VIPS_BLEND_MODE_OVER, VImage::option ()
				->set("x", (in.width() - top.width()) / 2)
				->set("y", (in.height() - top.height()) / 2)
			);
	}
