...
void CommandPerm::Deserialize(const json& content)
{
	...
	{
		if (p.value().contains("current")) this->_permissions[p.key()].first = p.value()["current"].get<int>();
		if (p.value().contains("default")) this->_permissions[p.key()].first = p.value()["default"].get<int>();
	}
}
